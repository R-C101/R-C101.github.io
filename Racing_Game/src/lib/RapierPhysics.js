 ...
 */
async function RapierPhysics( { frameRate = 60, selfStep = true } = {} ) {

	if ( RAPIER === null ) {

		RAPIER = await import( /* @vite-ignore */ RAPIER_PATH );
		await RAPIER.init();
	...
	// `dt` is optional. When omitted (legacy setInterval path) we derive it from
	// the internal Timer. When the caller drives stepping themselves they pass
	// the rAF delta directly — same value they use for vehicleController.updateVehicle.
	function step( dt ) {
	...
	// animate — only kick off the internal stepper when the caller has not
	// opted out (selfStep: false means they will call `step(dt)` themselves).
	if ( selfStep ) {
